...
const notAllowed: string[] = ['<script', '<style', '<!-- '];
// TODO: fix not allowed interval even if comment tag <!-- is followed by \n character

const notAllowedEnd: string[] = ['</script>', '</style>', '-->'];
...
function openHtmlFile(filename: string) {
    try {
...
function findNotAllowed(text: string, startingTag: string, endingTag: string): boolean{
...
function findNotAllowedIndexes(text: string, notValid: string[]): void {
...
function findParseTag(text: string, matchTag: string, start = 0) :boolean{
...
function findCloseTag(text: string, matchEndTag: string, start = 0): boolean {
...
function validateParseIndexes(indexes: number[], intervals: pair[]): number[] {

    const notValids: number[] = [];
...
function matchPairs(validIndexes: number[], validClosingIndexes: number[]): pair[] {

    const pairParse: pair[] = [];
    ...
    for (let cdx in validClosingIndexes) {
        for (let opx in validIndexes) {
            ...
            if (validClosingIndexes[cdx] > validIndexes[opx]) {
                ...
                validIndexes = validIndexes.filter(el => el !== validIndexes[parseInt(opx) - 1]);
                break;
            }
        }
    }
...
function removeChildren(pairParse: pair[]): pair[] {
...
function parseStringMaker(text:string, parseIntervals:pair[]): string[] {
